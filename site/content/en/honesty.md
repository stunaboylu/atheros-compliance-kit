...
"ISO/IEC 42001 compliance" would be the same false assurance in a different register. The Kit produces records for {{iso_clause_count}} named clauses and no others — [the FAQ lists them](faq.html), and every evidence pack `atheros-kit iso export` writes carries the list of what is *not* covered, so the document cannot be read as broader than it is once it leaves us. Clauses 4–7 and 10, the clause 9.2 internal audit programme, the clause 9.3 management review, the Statement of Applicability and the Annex A controls are management-system work, and no tool inside a CI pipeline performs them. One clause, 6.1.2, receives an input from us rather than being discharged by us, and is labelled that way wherever it appears. The same holds under the Act: the Kit evidences obligations, it does not determine conformity, and it is not a notified body.
